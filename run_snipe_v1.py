...
class SettlementSniper:
    ...
    def build_dashboard(self) -> Layout:
        ...
        header_text.append(f"| Size: ${self.config.size:.2f}", style="green")
        ...
        stats_text.append(f"Wins:    ", style="white")
        stats_text.append(f"{self.wins}", style="green")
        stats_text.append(f" ({win_rate:.0f}%)\n", style="dim")
        stats_text.append(f"Losses:  ", style="white")
        stats_text.append(f"{self.losses}\n", style="red")
    ...
    async def _execute_snipe(self, coin: str, market_slug: str, side: str, token_id: str,
        ...
        # Check coin-specific minimum size
        min_size = self.config.min_sizes.get(coin, 1.0)
        size_usd = max(self.config.size, min_size)

        # Validate we have enough balance
        if self.usdc_balance < size_usd:
            self.add_event(f"[{coin}] Insufficient balance (need ${size_usd:.2f})", "red")
            return

        # For display, show implied probability (for DOWN, show 1-price)
        display_prob = mid if side == "up" else (1 - mid)
        self.add_event(f"[{coin}] SNIPE {side.upper()} @ {display_prob:.1%}, {time_remaining:.0f}s left", "yellow")
        self.coin_states[coin].status = "sniping"
    ...
    async def _check_settlement(self, market_slug: str, tick_side: str, mid: float, time_remaining: float):
        """Check if a pending snipe has settled."""
        ...
        # Determine win: check if the position's own token resolved high
        # Only evaluate when we get a tick from the same side as our position
        if tick_side != snipe.side:
            return

        won = mid >= 0.95  # Our token at 95%+ means our side won

        if won:
            pnl = (1.0 - snipe.entry_price) * snipe.size_shares
            snipe.outcome = "win"
            snipe.pnl = pnl
            self.wins += 1
            self.total_pnl += pnl
            self.add_event(f"[{snipe.coin}] WIN +${pnl:.2f}", "green")
            self.coin_states[snipe.coin].status = "won"
        else:
            pnl = -snipe.entry_price * snipe.size_shares
            snipe.outcome = "loss"
            snipe.pnl = pnl
            self.losses += 1
            self.total_pnl += pnl
            self.add_event(f"[{snipe.coin}] LOSS ${pnl:.2f}", "red")
            self.coin_states[snipe.coin].status = "lost"

            # Stop bot if max losses reached
            if self.config.max_losses > 0 and self.losses >= self.config.max_losses:
                self.add_event(f"Max losses ({self.config.max_losses}) reached - stopping", "red bold")
                self.running = False

        self.completed_snipes.append(snipe)
        del self.pending_snipes[market_slug]

        # Don't redeem immediately - Polymarket needs time to mark as redeemable
        # The redeem loop will pick it up in 30-60 seconds
        ...
        while self.running:
            try:
                ...
                if positions:
                    ...
                    if redeemed > 0:
                        self.add_event(f"✅ Redeemed {redeemed} position(s)", "green bold")

                        # Update balance
                        ba = await self.bot.get_balance_allowance('', 'COLLATERAL')
                        new_balance = float(ba.get("balance", "0")) / 1_000_000
                        profit = new_balance - self.usdc_balance
                        self.usdc_balance = new_balance

                        if profit > 0:
                            self.add_event(f"💰 Balance: ${self.usdc_balance:.2f} (+${profit:.2f})", "green")
                    else:
    ...
    async def stop(self):
        ...
        console.print(Panel.fit(
            ...
            f"Wins: [green]{self.wins}[/green]\n"
            f"Losses: [red]{self.losses}[/red]\n"
